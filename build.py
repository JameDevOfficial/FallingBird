...
def create_windows_exe(love_file, love_version, arch, output_dir):
    ...
    game_exe = love_dir / "FallingBird.exe"
    
    print(f"Creating FallingBird.exe ({arch}-bit)...")
    ...
    # Create zip archive
    output_zip = output_dir / f"FallingBird-win{arch}.zip"
...
def main():
    ...
    web = releases / "web"
    love_file = releases / "FallingBird.love"
    ...
    checks = {
        ".love file": love_file,
        "Windows 32-bit": executables / "FallingBird-win32.zip",
        "Windows 64-bit": executables / "FallingBird-win64.zip", 
